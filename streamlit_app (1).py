...
class DispositionClassifier:
    ...
    def classify_return(self, condition: str, category: str, value: float,
        ...
        condition_weights = {
            ...
            'Unknown': {'Dispose': 0.5, 'Liquidate': 0.5}
        }
...
class ReverseRoutingOptimizer:
    ...
    def _select_optimal_hub(self, origin: str, urgency: float) -> str:
        ...
        for hub in self.hubs:
            ...
            weight_cost = 0.4 if urgency < 0.7 else 0.2
            weight_speed = 0.3 if urgency >= 0.7 else 0.2
            weight_carbon = 0.3
...
')
        for line in lines:
            if 'RECOMMENDATION:' in line:
                parts = line.split('|')
                if len(parts) >= 3:
                    rec = {'title': parts[0].split('RECOMMENDATION:')[1].strip(), 'detail': parts[1].split('DETAIL:')[1].strip() if 'DETAIL:' in parts[1] else '', 'impact': parts[2].split('IMPACT:')[1].strip() if 'IMPACT:' in parts[2] else ''}
                    if 'disposition' in rec['title'].lower() or 'review' in rec['title'].lower():
                        suggestions['disposition_optimization'].append(rec)
                    elif 'routing' in rec['title'].lower() or 'cost' in rec['title'].lower():
    ...
    def _mock_suggestions(self, context: Dict) -> Dict:
        manual_review_rate = context.get('manual_review', 0) / max(context.get('total_returns', 1), 1)
        return {
...
def generate_sample_data(n: int = 100) -> pd.DataFrame:
    ...
    data = {'return_id': [f'RET-{1000+i}' for i in range(n)], 'product_category': np.random.choice(categories, n), 'condition': np.random.choice(conditions, n, p=[0.15, 0.25, 0.30, 0.20, 0.10]), 'return_reason': np.random.choice(reasons, n), 'original_value': np.random.uniform(20, 500, n).round(2), 'origin_location': np.random.choice(locations, n), 'return_date': [datetime.now() - timedelta(days=int(np.random.randint(0, 30))) for _ in range(n)], 'urgency_score': np.random.uniform(0.3, 1.0, n).round(2)}
...
def main():
    ...
    with st.sidebar:
        ...
        # Prefer Streamlit secrets (secure) and fall back to environment variables
        api_key = None
        try:
...
- 📉 Avg confidence: {processed_data['confidence'].mean():.1%}")
    ...
    with tab_map:
        ...
        if df_map.empty:
            ...
            # We'll aggregate origins to avoid marker spam by grouping and choosing the most common mode per origin
    ...
    with tab3:
        ...
        if st.button("🔮 Generate AI Suggestions", use_container_width=True):
            with st.spinner("Consulting Gemini AI..."):
                context = {
                    ...
                    'manual_review': processed_data['needs_review'].sum() if 'needs_review' in processed_data.columns else 0,
        ...
        if st.session_state.ai_suggestions:
            ...
            for i, suggestion in enumerate(suggestions.get('disposition_optimization', []), 1):
                with st.expander(f"💡 {suggestion['title']}", expanded=(i==1)):
                    st.markdown(f"**Details:** {suggestion['detail']}")
